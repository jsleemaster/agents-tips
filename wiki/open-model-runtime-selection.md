...
- For live voice runtimes, test interruption recovery, background-noise rejection, wake or pause behavior, and transcript alignment in realistic mobile or room scenarios; clean sample conversations are weaker evidence than sustained turn-taking under messy audio.
...
- For local SOTA-LLM experiments, treat VRAM or unified-memory capacity as only the first gate: memory bandwidth, context length at target latency, thermal limits, driver/runtime maturity, and task quality under complex prompts decide whether the box is a production candidate or just a benchmark machine.
