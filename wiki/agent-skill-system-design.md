...
- For repo or issue-driven coding agents, do not let public, unauthenticated, or lower-trust collaboration surfaces directly influence private-repo reads or writes; package trust-zone separation, org/repo allowlists, prompt-injection tests, redaction rules, and exfiltration canaries with the workflow.
- Cross-repository agent workflows should require an explicit source-to-target policy: which public issues, comments, PRs, or docs can become instructions, which private files can be read, and what content may be quoted back across the boundary.
...
- For model-native computer use that spans desktop, mobile, and browser surfaces, package sandbox accounts, least-privilege credentials, approval checkpoints, screen-state logs, retry reasons, and safe-stop behavior before treating the model's GUI ability as a production workflow.
...
- For physical-system, defense, or field-deployed agent skills, package data-location rules, offline and degraded-network behavior, sensor or actuator boundaries, human authorization, procurement/security certification, maintenance ownership, and post-action audit evidence with the workflow.
- For embodied-navigation or robotics skills, package sensor assumptions, map/control-layer boundaries, observation-action logs, route deviation, near-miss evidence, fail-safe activations, human intervention rate, and recovery traces; simulation success should not replace site-level safety and domain-shift evaluation.
...
- Antibody-design or wet-lab discovery skills should expose stage gates, not only model predictions: target assay, hit-to-lead criteria, wet-lab iteration count, developability failures, experimental-result ingestion, design rationale, and regulatory traceability belong in the reusable workflow package.
...
- For multimodal media-generation skills, package search and code-tool authority, iteration limits, generated-asset provenance, watermark verification, edit history, and human review thresholds; prompt-to-image quality alone is too narrow once the model can call tools and self-refine.
- For generated-ad or commerce-creative skills, package channel-specific disclosure rules, asset metadata, prompt and model lineage, edit and approval history, claim verification, brand-safety review, and distribution evidence; user-facing AI labels do not replace truthfulness checks.
...
- Code-review agent packages need task-shaped tool instructions and golden PR sets, not only generic repo search access; preserve diff anchors, expected findings, false-positive policy, accepted-finding cost, first-review latency, review-cycle count, and tool-trace budget so instruction changes can be A/B tested without confusing review quality with cheaper exploration.
...
- For agentic RL workflow packages, define the environment, verifier, reward source, failure taxonomy, deployment telemetry, and rollback gates before training or routing; vendor benchmarks should be reproduced against the same production traces that will drive improvement.
...
- For globally exposed AI products, package governance evidence as versioned workflow artifacts: system cards, model/version changes, eval and red-team results, deployment rationale, privacy/security reviews, environmental notes, user-impact analysis, and escalation decisions should be reusable when policy, procurement, or audit questions arrive.
